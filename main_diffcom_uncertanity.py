...
def evaluate_latent_correlation(uncertainty_map, x_recon, input_image, operator):
    """
    不確かさマップ(Pixel)と、潜在空間での誤差(Latent Error)の相関を計算する関数
    ※ 平坦化された潜在表現を元の空間形状に戻して計算します
    """
    ...
    # サイズが異なる場合のみリサイズ (Average Poolingでダウンサンプリング)
    target_size = latent_error_map.shape[-2:]
    
    if u_map.shape[-2:] != target_size:
        try:
            u_map_resized = torch.nn.functional.adaptive_avg_pool2d(
                u_map, output_size=target_size
            )
        except Exception as e:
            print(f"Resize failed: u_map {u_map.shape} -> target {target_size}. Error: {e}")
            return 0.0, None, None
    else:
        u_map_resized = u_map
    ...
    for idx, batch in enumerate(dataloader):
        ...
        if u_map is not None:
            ...
            if lat_err is not None:
                ...
                # 1. 潜在空間でのエラーマップ (Latent Error Map)
                ...
                # 2. リサイズされた不確かさマップ (Resized Uncertainty Map)
                ...
                logger.info(f"Batch {idx}: Latent correlation skipped (shape mismatch or invalid).")
            
            # メモリリーク防止
            diffcom_module.latest_uncertainty_map = None
        else:
            logger.info(f"Batch {idx}: No uncertainty map found.")
        # =========================================================
