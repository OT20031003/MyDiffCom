...
def calculate_psnr_from_disk(base_path, device):
    """
    保存済みの画像からPSNRを計算する (Single_Run対応版)
    """
    ...
    print(f"Processing PSNR for {len(batch_dirs)} samples from: {folder_name}")
    ...
    # 平均算出
    final_results = {}
    print(f"--- PSNR Results ({folder_name}) ---")
    for m in methods:
    ...
    # 保存
    output_json = os.path.join(base_path, "post_process_psnr_singlerun.json")
...
if __name__ == "__main__":
    ...
    for snr_val in SNR_LABELS:
        ...
        for gamma_val in GAMMA_LIST:
            ...
            # 存在チェックをしてから実行（パス間違いなどを防ぐため）
            if os.path.exists(target_path):
                calculate_psnr_from_disk(target_path, device)
            else:
