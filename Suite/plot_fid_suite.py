...
# 対象メトリクス
TARGET_METRIC_KEY = "fid" # JSON内のキーはファイル名生成ロジックによりトップレベルにある場合が多いが、関数内で処理
...
def load_fid_data(targets, patterns):
    """
    ...
    for root in roots_to_scan:
        ...
        # 再帰的に post_process_fid.json を探す
        search_path = os.path.join(root, "**", "post_process_fid.json")
        ...
        for fpath, (snr, content) in file_cache.items():
            if fpath.startswith(source_root):
                if method in content:
                    ...
                    elif isinstance(val, dict) and "fid" in val: # 万が一構造が違う場合
                        data_store[method][snr] = val["fid"]
...
def plot_graph(data_store):
    ...
    #ax.set_title(f"FID Comparison {title_suffix} - {DATASET}", fontsize=15, fontweight='bold')
    ...
    filename = f"Suite/fid_comparison_suite_{VERSION}.png"
...
if __name__ == "__main__":
    print(f"=== Plotting FID Suite (Mode: {VERSION}) ===")
    ...
    # 2. データ読み込み
    data = load_fid_data(targets, patterns)
