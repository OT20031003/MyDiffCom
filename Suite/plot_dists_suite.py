...
# 対象メトリクス
TARGET_METRIC_KEY = "dists" # JSON内のキーはファイル名生成ロジックによりトップレベルにある場合が多いが、関数内で処理
...
def load_dists_data(targets, patterns):
    """
    ...
    for root in roots_to_scan:
        ...
        # 再帰的に post_process_dists.json を探す
        search_path = os.path.join(root, "**", "post_process_dists.json")
        ...
        for fpath, (snr, content) in file_cache.items():
            if fpath.startswith(source_root):
                if method in content:
                    ...
                    elif isinstance(val, dict) and "dists" in val: # 万が一構造が違う場合
                        data_store[method][snr] = val["dists"]
...
def plot_graph(data_store):
    ...
    #ax.set_title(f"DISTS Comparison {title_suffix} - {DATASET}", fontsize=15, fontweight='bold')
    ...
    filename = f"Suite/dists_comparison_suite_{VERSION}.png"
...
if __name__ == "__main__":
    print(f"=== Plotting DISTS Suite (Mode: {VERSION}) ===")
    ...
    # 2. データ読み込み
    data = load_dists_data(targets, patterns)
