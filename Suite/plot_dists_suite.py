        ...
        for fpath, (snr, content) in file_cache.items():
            ...
            if os.path.abspath(fpath).startswith(os.path.abspath(source_root)):
                ...
                if method in content:
                    val = content[method]
                    # DISTSはフラットな数値か、{"dists": val}形式の可能性があるため両対応
                    if isinstance(val, (float, int)):
                         data_store[method][snr] = val
                    elif isinstance(val, dict) and TARGET_METRIC_KEY in val: 
...
def plot_graph(data_store):
    ...
    filename = f"{save_dir}/dists_comparison_suite_{VERSION}.png"
