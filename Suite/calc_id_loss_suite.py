...
def calculate_id_loss_for_snr(target_path, id_model, device, target_methods):
    """
    ...
    # 結果コンテナ
    id_scores = {m: [] for m in all_methods}     # Cosine Similarity
    id_losses = {m: [] for m in all_methods}     # 1 - Similarity
    ...
    for m in all_methods:
        ...
        if len(losses) > 0:
            ...
            print(f"{display_name:<30} | {avg_loss:.4f}     | {avg_score:.4f}")
        else:
            print(f"{display_name:<30} | N/A        | N/A")
...
if __name__ == "__main__":
    ...
    for snr_label in SNR_LABELS:
        ...
        calculate_id_loss_for_snr(target_path, id_model, device, TARGET_METHODS)