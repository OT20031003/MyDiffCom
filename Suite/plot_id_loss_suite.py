        ...
        for fpath, (snr, content) in file_cache.items():
            if fpath.startswith(source_root):
                if method in content:
                    # id_lossキーの直下の値、またはネストされた辞書から取得
                    val = content[method]
...
def plot_graph(data_store):
    ...
    #ax.set_title(f"ID Loss Comparison {title_suffix} - {DATASET}", fontsize=15, fontweight='bold')
    ...
    filename = f"Suite/id_loss_comparison_suite_{VERSION}.png"
