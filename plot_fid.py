...
STYLE_CONFIG = {
    "1_JSCC_Init":               {"color": "black", "linestyle": ":",  "marker": "x"}, # 初期JSCC: 黒点線
    "2_Phase1_Recon":            {"color": "blue",  "linestyle": "-",  "marker": "o"}, # Phase1: 青実線
    
    # Temporal系: 緑色
    "3_P2_temporal_raw_Unc":     {"color": "green", "linestyle": "-",  "marker": "^"}, # Unc: 実線・三角
    "3_P2_temporal_raw_Sem":     {"color": "green", "linestyle": "--", "marker": "v"}, # Sem: 破線・逆三角
    
    # Perturbation系: 赤色
    "3_P2_perturbation_raw_Unc": {"color": "red",   "linestyle": "-",  "marker": "s"}, # Unc: 実線・四角
    "3_P2_perturbation_raw_Sem": {"color": "red",   "linestyle": "--", "marker": "D"}, # Sem: 破線・ダイヤ
    
    # Random: 灰色
    "3_P2_Random":               {"color": "gray",  "linestyle": "-.", "marker": "d"}, # Random: 一点鎖線
}
...
def load_fid_data_recursive():
    """
    指定ディレクトリ以下の post_process_fid.json を探索し、
    ...
    # SNR抽出用正規表現: awgn_-4dB, awgn_02dB, awgn_2.5dB などに対応
    ...
    for fpath in files:
        ...
        try:
            ...
            # データ構造: data_store[snr][method] = fid_score
            if snr not in data_store:
                data_store[snr] = {}

            for method in TARGET_METHODS:
                if method in content:
                    data_store[snr][method] = content[method]
