...
def evaluate_correlations(uncertainty_map, x_recon, input_image, operator):
    """
    不確かさマップと、Latent誤差およびPixel誤差との相関を包括的に計算する関数
    戻り値: (results_dict, latent_error_map, pixel_error_map, u_map_lat, u_map_pix)
    """
    ...
    if z_gt is not None:
        # Latent Error (Channel Mean MSE) -> [1, H', W']
        latent_error_map = torch.mean((z_gt - z_recon) ** 2, dim=1).detach().cpu()
        print(f"latent_error_map.shape = {latent_error_map.shape}")
        e_flat_lat = latent_error_map.flatten().numpy()

        # Resize Uncertainty to Latent Size
        target_size_lat = latent_error_map.shape[-2:]
        if u_map.shape[-2:] != target_size_lat:
             u_map_lat = torch.nn.functional.interpolate(u_map, size=target_size_lat, mode='bilinear', align_corners=False)
        else:
             u_map_lat = u_map

        # (A) Latent Mean
        u_mean_lat = torch.mean(u_map_lat, dim=1).flatten().numpy()
        results['latent_mean'] = calc_corr(u_mean_lat, e_flat_lat)

        # (B) Latent RGB (Compare U_c with Latent_Mean_Error)
        if u_map_lat.shape[1] == 3:
            for i, name in enumerate(['R', 'G', 'B']):
                u_c = u_map_lat[:, i, :, :].flatten().numpy()
                results[f'latent_{name}'] = calc_corr(u_c, e_flat_lat)
    else:
        latent_error_map = None
        u_map_lat = None

    # =========================================================
    # 2. Pixel Space Correlation
    # =========================================================
    # Pixel Error Map [1, 3, H, W]
    pixel_error_map_3c = ((x_recon - input_image) ** 2).detach().cpu()
    print(f"pixel_error_map_3c.shape = {pixel_error_map_3c.shape}")
    # Resize Uncertainty to Pixel Size (Original Image Size)
    target_size_pix = pixel_error_map_3c.shape[-2:]
    if u_map.shape[-2:] != target_size_pix:
    ...
    results['pixel_mean'] = calc_corr(u_mean_pix, e_mean_pix)

    # (B) Pixel RGB (Direct Comparison: U_R vs Error_R)
    if u_map_pix.shape[1] == 3 and pixel_error_map_3c.shape[1] == 3:
        for i, name in enumerate(['R', 'G', 'B']):
            u_c = u_map_pix[:, i, :, :].flatten().numpy()
            e_c = pixel_error_map_3c[:, i, :, :].flatten().numpy()
            results[f'pixel_{name}'] = calc_corr(u_c, e_c)

    return results, latent_error_map, pixel_error_map_3c, u_map_lat, u_map_pix
...
def p_sample_loop(config, noise_schedule, unet, diffusion, operator, cond_method, dataloader, device, logger):
    ...
    try:
        for idx, batch in enumerate(dataloader):
            ...
            # =========================================================
            # 不確かさの相関評価 (Latent Space & Pixel Space)
            # =========================================================
            u_map = diffcom_module.latest_uncertainty_map
            
            if u_map is not None:
                # Latent & Pixel Correlation Calculation
                corr_dict, lat_err, pix_err, u_map_lat, u_map_pix = evaluate_correlations(
                ...
                if corr_dict:
                    ...
                    logger.info(f"Batch {idx}: Correlations -> {log_corr_str}")
                    
                    # Add to JSON
                    for k, v in corr_dict.items():
                        batch_record["uncertainty"][f"corr_{k}"] = float(v)
                    ...
                    if lat_err is not None:
                        ...
                        plt.imsave(os.path.join(save_debug_path, 'error_latent.png'), e_vis, cmap='jet')
                    
                    # 2. Pixel Error (Mean)
                    if pix_err is not None:
                        # Mean Pixel Error
                        pe_vis = torch.mean(pix_err, dim=1)[0].numpy()
                        pe_vis = (pe_vis - pe_vis.min()) / (pe_vis.max() - pe_vis.min() + 1e-8)
                        plt.imsave(os.path.join(save_debug_path, 'error_pixel_mean.png'), pe_vis, cmap='gray')

                        # RGB Pixel Errors
                        for c, name in enumerate(['R', 'G', 'B']):
                             pe_c = pix_err[0, c].numpy()
                             pe_c = (pe_c - pe_c.min()) / (pe_c.max() - pe_c.min() + 1e-8)
                             plt.imsave(os.path.join(save_debug_path, f'error_pixel_{name}.png'), pe_c, cmap='gray')

                    # 3. Uncertainty Maps
                    if u_map_pix is not None:
                        # Mean Uncertainty
                        u_vis = torch.mean(u_map_pix, dim=1)[0].numpy()
                        u_vis = (u_vis - u_vis.min()) / (u_vis.max() - u_vis.min() + 1e-8)
                        plt.imsave(os.path.join(save_debug_path, 'uncertainty_mean.png'), u_vis, cmap='jet')
                        
                        # RGB Uncertainties
                        if u_map_pix.shape[1] == 3:
                            for c, name in enumerate(['R', 'G', 'B']):
                                u_c = u_map_pix[0, c].numpy()
                                u_c = (u_c - u_c.min()) / (u_c.max() - u_c.min() + 1e-8)
                                plt.imsave(os.path.join(save_debug_path, f'uncertainty_{name}.png'), u_c, cmap='jet')
